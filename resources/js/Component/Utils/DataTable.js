...
const DataTable = ({ columns, fetchUrl, history, linkDetail, linkPagination }) => {
    ...
    const handleDetail = (data) => {
        history.push(`${linkDetail}/${data.id}`);
    }
        ...
        <div>
            ...
            <div className="row mb-3">
                ...
                <div className="col-md-2">
                    <div className="input-group">
                        <label className="mt-2 mx-2 form-label">Per page</label>
                        <select className="form-select w-50" value={perPage} onChange={(e) => handlePerPage(e.target.value)}>
                ...
                <table className="table table-hover">
                    <thead className="table-dark">
                        <tr>
                            {columns.map((column) => {
                                return (
                                    <th key={column} onClick={(e) => handleSort(column)} style={{cursor:'pointer'}}>
                        ...
                        {!loading ? (
                            data.map((d, index) => {
                                return (
                                    <tr key={index}>
                                        {columns.map((column) => {
                                            return <td key={column} style={{cursor:'pointer'}} onClick={() => handleDetail(d)}>{d[column]}</td>
                                        })}
                                    </tr>
                                )
                            })
